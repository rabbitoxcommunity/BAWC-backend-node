...
import { deleteCategory, getCategories } from '../../redux/actionCreator';
...
export default function ManageCategory() {
  ...
  const columns = [
    ...
    { name: 'Category Name', selector: row => row.name, sortable: true, width: '20%' },
    {
      name: 'Image',
      width: "40%",
  ...
  const deleteItem = () => {
    dispatch(deleteCategory({ id: selectedID }, (res) => {
      if (res?.status == 200) {
        setShow(false)
        setSelectedID('')
        fetchCategories()
  ...
  return (
    <Page>
      ...
      <Page.Body className={'card'}>
        <h2 className="title mb-4"><span>All Categories</span></h2>
